...
- **Visual examples by default.** The learner learns better when concepts come
  ...
  tie them directly to the operation being explained. Record this preference in
  each topic's `NOTES.md` as well.
- Keep lessons short and single-win; stay in working memory (see `MISSION.md`).
- Show concept → show Odin/raylib code → small interactive check → prompt a
  follow-up question.